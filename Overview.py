...
st.set_page_config(
    page_title="Hospitality Intelligence Dashboard",
    page_icon="🏨",
    layout="wide",
...
with st.sidebar:
    ...
    st.caption("Global Market Analytics · MongoDB")
    st.markdown("---")
    ...
    sel_property = st.selectbox("Type", property_types)

    room_types = ["All"] + sorted(df["room_type"].dropna().unique().tolist())
    sel_room = st.selectbox("Room", room_types)

    price_min, price_max = int(df["price"].min()), int(df["price"].quantile(0.98))
    sel_price = st.slider("Price ($)", price_min, price_max, (price_min, price_max))
...
# ── Header ───────────────────────────────────────────────────────────────────
tu.airbnb_header("Global Hospitality Intelligence", "Comprehensive market analytics and performance benchmarking powered by MongoDB")
