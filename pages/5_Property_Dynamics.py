...
st.set_page_config(page_title="Features · Hospitality Intelligence", page_icon="🏡", layout="wide")
...
with st.sidebar:
    ...
    sel_pt = st.selectbox("Property",
                          ["All"]+sorted(df["property_type"].dropna().unique().tolist()))
    sel_rt = st.selectbox("Room",
...
# ── Header ─────────────────────────────────────────────────────────────────────
tu.airbnb_header("Amenities & Features", "Common amenities, price impact analysis, and property specifications")
