...
st.set_page_config(page_title="Hosts · Hospitality Intelligence", page_icon="👤", layout="wide")
...
with st.sidebar:
    ...
    sel_super = st.selectbox("Status", ["All", "Superhost", "Regular"])
    sel_verified = st.checkbox("Verified Identity", False)

fdf = df.copy()
if sel_c != "All":   fdf = fdf[fdf["country"] == sel_c]
if sel_super == "Superhost":     fdf = fdf[fdf["is_superhost"]==True]
if sel_super == "Regular":       fdf = fdf[fdf["is_superhost"]==False]
if sel_verified:     fdf = fdf[fdf["host_identity_verified"]==True]

# ── Header ─────────────────────────────────────────────────────────────────────
tu.airbnb_header("Host Insights", "Performance analytics for hosts, superhost impact, and multi-listing scale")
